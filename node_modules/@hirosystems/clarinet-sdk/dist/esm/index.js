...
import { CallContractArgs, DeployContractArgs, TransferSTXArgs, ContractOptions, } from "@hirosystems/clarinet-sdk-wasm";
    ...
    get(session, prop, receiver) {
        ...
        if (prop === "callReadOnlyFn" || prop === "callPublicFn") {
            const callFn = (contract, method, args, sender) => {
                const response = session[prop](new CallContractArgs(contract, method, args.map((a) => Cl.serialize(a)), sender));
...
function memoizedInit() {
    ...
    return async (manifestPath = "./Clarinet.toml", noCache = false) => {
        if (noCache || !simnet) {
            ...
            simnet = new Proxy(new module.SDK(vfs), getSessionProxy());
        }
